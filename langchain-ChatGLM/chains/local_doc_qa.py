...
from langchain.document_loaders import UnstructuredFileLoader, TextLoader, CSVLoader, PyPDFLoader
...
from vectorstores.MyElasticVectorSearch import MyElasticVectorSearch
...
def md_add_info(md_file):
    ...
    for line in html.split("\n"):
        ...
        if line.startswith("<p"):
            element = file_name.split(".md")[0] + "  " + "  ".join(list(dict(sorted(pre_level.items())).values())[0:latest_level]) + "   " + SPECIAL_FLAG + line_text
            metadata = {
                "source": md_file,
                "filename": file_name,
...
class LocalDocQA:
    ...
    def init_cfg(self,
        ...
        # self.elasticsearch_url = f"https://elastic:elastic@10.1.226.1:9200"
        self.elasticsearch_url = ELASTICSEARCH_URL
        self.translation = pipeline("translation", model=AutoModelForSeq2SeqLM.from_pretrained("Helsinki-NLP/opus-mt-zh-en"), tokenizer=AutoTokenizer.from_pretrained("Helsinki-NLP/opus-mt-zh-en"))

    def get_es_index(self):
        es_client = Elasticsearch(hosts=[self.elasticsearch_url], verify_certs=False)
        r = es_client.cat.indices(index="fg_*", format="json")
        list_index = []
        for raw in r.raw:
            list_index.append(raw["index"])
        return list_index

    def es_init_knowledge_vector_store(self,
        ...
        if len(docs) > 0:
            if vs_path not in self.get_es_index():
                create_es_index(index=vs_path)
            ...
            torch_gc()
            # if vs_path in self.get_es_index():
    ...
    def es_one_knowledge_add(self, vs_path, one_title, one_conent, one_content_segmentation, sentence_size):
        try:
    ...
    def get_knowledge_based_answer_by_qdrant(self, query, chat_history=[], streaming: bool =STREAMING):
        client = qdrant_client.QdrantClient("10.1.226.1", port=6333)
        vector_store = Qdrant(client, "fg", self.embeddings)
    ...
    def get_knowledge_based_answer_by_es(self, query, vs_path, chat_history=[], streaming: bool =STREAMING):
        ...
        query2 = self.translation(query)[0]['translation_text']
        related_docs_with_score = vector_store.similarity_search_with_score(query=query, query2=query2 , k=self.top_k, filter={""})
        torch_gc()
        if len(related_docs_with_score) > 0:
            prompt = generate_prompt(related_docs_with_score, query)
        else:
        ...
        prompt = prompt.replace(SPECIAL_FLAG, "")
        for answer_result in self.llm.generatorAnswer(prompt=prompt, history=chat_history,
                                                      streaming=streaming):
            resp = answer_result.llm_output["answer"]
            history = answer_result.history
    ...
    def get_knowledge_based_answer_by_es_special(self, query, vs_path, chat_history=[], streaming: bool =STREAMING):
        vector_store = MyElasticVectorSearch(
            elasticsearch_url=self.elasticsearch_url,
            index_name=vs_path,
            embedding=self.embeddings,
    ...
    def get_knowledge_based_answer_by_es_files(self, query, vs_path, chat_history=[], file_list=[], streaming: bool =STREAMING):
        vector_store = MyElasticVectorSearch(
            elasticsearch_url=self.elasticsearch_url,
            index_name=vs_path,
            embedding=self.embeddings,
        ...
        related_docs_with_score = vector_store.all_search_with_score(query, k=10000, file_list=file_list)
        torch_gc()
        if len(related_docs_with_score) > 0:
            prompt = generate_prompt(related_docs_with_score, query)
        else:
    ...
    def es_list_file_from_vector_store(self, vs_path):
        ...
        data = es_client.search(index=vs_path, body=query)
        docs = data.raw["aggregations"]["source_aggs"]["buckets"]
        return [os.path.split(doc["key"])[-1] for doc in docs]
