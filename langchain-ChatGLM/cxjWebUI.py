...
def get_vs_list():
    ...
    # return lst_default + list_index
    return ["法律法规", "俄乌冲突", "台海问题"]
...
def get_vector_store(vs_id, files, sentence_size, history, one_conent, one_content_segmentation):
    ...
    return vs_path, None, history + [[None, file_status]], \
           gr.update(choices=local_doc_qa.es_list_file_from_vector_store(name_to_index[vs_path]) if vs_path else [])
...
def delete_vs(vs_id, chatbot):
    try:
        shutil.rmtree(os.path.join(ES_ROOT_PATH, vs_id))
...
知识库问答模式，选择知识库名称后，即可开始问答，当前知识库为：{default_vs}，还可以选择俄乌冲突与台海问题知识库进行问答。
    ...
    with gr.Tab("对话"):
        with gr.Row():
            ...
            with gr.Column(scale=5):
                ...
                with vs_setting:
                    vs_refresh = gr.Button("更新已有知识库选项", visible=False)
                    ...
                    with file2vs:
                        ...
                        with gr.Tab("选择文件"):
                            files_to_delete = gr.CheckboxGroup(choices=[],
                                                             ...
                                                             interactive=True)
                            delete_file_button = gr.Button("从知识库中删除选中文件", visible=False)
