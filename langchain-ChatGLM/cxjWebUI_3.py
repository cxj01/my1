...
def get_answer(query, vs_path, history, mode, files_to_delete, score_threshold=VECTOR_SEARCH_SCORE_THRESHOLD,
    ...
    if mode == "Bing搜索问答":
        ...
        if len(files_to_delete) == 0:
            if query[-1] == "、":
                for resp, history in local_doc_qa.get_knowledge_based_answer_by_es_special(
        ...
        if "你好" in query and len(query) < 4:
            s = "你好👋！我是人工智能助手HF-GPT，很高兴见到你，欢迎问我任何问题。"
            history += [[]]
            pre = ""
            for ans in iter(s):
                t_resp = ans
                ...
                history[-1][-1] = pre
                yield history, ""
        elif "你是谁" in query and len(query) < 6:
            s = "我是一个名为 HF-GPT 的人工智能助手，是南京华飞数据技术有限公司于 2023 年训练的语言模型。我的任务是针对用户的问题和要求提供适当的答复和支持。"
            history += [[]]
            pre = ""
            for ans in iter(s):
                t_resp = ans
